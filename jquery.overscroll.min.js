...
undefined;c.toggleThumbs(a.data,false);a.data.target.data("dragging",false);a.data.options.onDriftEnd.call(a.data.target,a.data)},c.constants.timeout);return false},moveThumbs:function(a,b,d,g,e,h,i){if(a.data.options.showThumbs){g=a.data.thumbs;e=a.data.sizing;if(g.horizontal){h=b*(1+e.container.width/e.container.scrollWidth);i=d+e.thumbs.horizontal.top;g.horizontal.css("margin",i+"px 0 0 "+h+"px")}if(g.vertical){h=b+e.thumbs.vertical.left;i=d*(1+e.container.height/e.container.scrollHeight);g.vertical.css("margin",
i+"px 0 0 "+h+"px")}}},start:function(a){c.clearInterval();if(!f(a.target).is(a.data.options.cancelOn)){c.normalizeEvent(a);a.data.target.bind(c.events.drag,a.data,c.drag).stop(true,true).data("dragging",false);a.data.position=c.setPosition(a,{});a.data.capture=c.setPosition(a,{},2);c.triggerEvent("dragstart",a.data);return false}},drag:function(a){c.normalizeEvent(a);a.data.target.data("dragging")||c.toggleThumbs(a.data,true);if(a.data.options.direction!=="vertical")this.scrollLeft-=a.pageX-a.data.position.x;
if(a.data.options.direction!=="horizontal")this.scrollTop-=a.pageY-a.data.position.y;c.moveThumbs(a,this.scrollLeft,this.scrollTop);c.setPosition(a,a.data.position);if(--a.data.capture.index<=0){a.data.target.data("dragging",true);c.setPosition(a,a.data.capture,c.constants.captureThreshold)}return true},normalizeEvent:function(a){if(c.checkIosDevice()){var b=a.originalEvent.changedTouches[0];a.pageX=b.pageX;a.pageY=b.pageY}},stop:function(a){if(a.data.position){a.data.target.unbind(c.events.drag,
c.drag);c.triggerEvent("dragend",a.data);a.data.target.data("dragging")?c.drift(this,a,function(b){b.target.data("dragging",false);b.options.onDriftEnd.call(b.target,b);c.toggleThumbs(b,false)}):c.toggleThumbs(a.data,false);a.data.capture=a.data.position=undefined}return!a.data.target.data("dragging")},clearInterval:function(){c.driftInterval&&l.clearInterval(c.driftInterval)},setInterval:function(a){c.driftInterval=a},drift:function(a,b,d){c.normalizeEvent(b);var g=b.data.options.scrollDelta*(b.pageX-
b.data.capture.x),e=b.data.options.scrollDelta*(b.pageY-b.data.capture.y),h=a.scrollLeft,i=a.scrollTop,j=g/c.constants.driftSequences,k=e/c.constants.driftSequences,o=c.constants.driftDecay;if(b.data.options.direction!=="vertical")h-=g;if(b.data.options.direction!=="horizontal")i-=e;c.triggerEvent("driftstart",b.data);c.setInterval(l.setInterval(function(){var m=true;if(k>1&&a.scrollTop>i||k<-1&&a.scrollTop<i){m=false;a.scrollTop-=k;k/=o}if(j>1&&a.scrollLeft>h||j<-1&&a.scrollLeft<h){m=false;a.scrollLeft-=
j;j/=o}c.moveThumbs(b,a.scrollLeft,a.scrollTop);if(m){c.clearInterval();c.triggerEvent("driftend",b.data);d.call(null,b.data)}},c.constants.driftFrequency))},getSizing:function(a,b){b={};b.container={width:a.width(),height:a.height()};a.scrollLeft(c.constants.boundingBox).scrollTop(c.constants.boundingBox);b.container.scrollWidth=a.scrollLeft();b.container.scrollHeight=a.scrollTop();a.scrollTop(0).scrollLeft(0);b.thumbs={horizontal:{width:b.container.width*b.container.width/b.container.scrollWidth,
...
width:a.width+"px",height:a.height+"px",margin:a.top+"px 0 0 "+a.left+"px","-moz-border-radius":a.corner+"px","-webkit-border-radius":a.corner+"px","border-radius":a.corner+"px"}}})})(window,Math,jQuery);
